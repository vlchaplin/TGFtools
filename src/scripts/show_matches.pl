...
sub _usage_info() {
    print "Usage: show_matches.pl [-s delimiter] [-w min_width] [-W where] [--desc --help]  [column names]\n";
...
my ($all_fields, $format, $units) = _get_columns($dbh, $assoc_tbl, \@sql_col_expr);

if ( $desc_and_quit ) {
 
    print "----------------------------------------\n";
    print "Database: '$tgf_db',  Table: '$assoc_tbl'\n";
    print "----------------------------------------\n";
...
@sql_col_expr = @$all_fields unless @sql_col_expr;

my $stmt = "SELECT ".join(',', @sql_col_expr)." from $assoc_tbl $whr";
my $result = $dbh->selectall_arrayref($stmt);

unless ( $result && @$result ) {
    print "No matches are currently defined.\nTry running 'get_match_coords'.\n\n";
    exit;
}
